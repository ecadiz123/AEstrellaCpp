...
public:
	...
	Punto Arriba(const Punto& actual) 
	{ return Punto(actual.i - 1, actual.j); }
	
	Punto Abajo(const Punto& actual)
	{ return Punto(actual.i + 1, actual.j); }
	
	Punto Izquierda(const Punto& actual) 
	...
	void printFormatoEvaluador()
	{
		...
		//Nodos Visitados
		salidatxt << this->salida.size() << endl;
		//for que imprime las direcciones a la salida
